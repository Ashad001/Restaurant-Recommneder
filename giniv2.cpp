...
{
	int indarr[5];
	for(int i=0;i<arrlen;i++){//loop to find where instance starts in a container of generic size
	...
	for(int i=0;i<arrlen;i++){
		if(alldata[i]!="Reservation"){//loop to find where instance starts in a container of generic size
			instance_index++;
		}
		else if(alldata[i]=="Reservation"){
	...
	for(int i=0;i<arrlen;i++){
		if(alldata[i]!="Price Range"){//loop to find where instance starts in a container of generic size
			instance_index++;
		}
		else if(alldata[i]=="Price Range"){
	...
	for(int i=0;i<arrlen;i++){
		if(alldata[i]!="Cuisine"){//loop to find where instance starts in a container of generic size
			instance_index++;
		}
		else if(alldata[i]=="Cuisine"){
	...
	for(int i=0;i<arrlen;i++){
		if(alldata[i]!="Timing"){//loop to find where instance starts in a container of generic size
			instance_index++;
		}
		else if(alldata[i]=="Timing"){
	...
	((n_instances[0]/all_instances)*(1-pow((recount[1]/(recount[1]+drecount[1])),2)-pow((drecount[1]/(recount[1]+drecount[1])),2)))+(y_instances[1]/all_instances)*
	(1-pow((recount[2]/(recount[2]+drecount[2])),2)-pow((drecount[2]/(recount[2]+drecount[2])),2))+
	((n_instances[1]/all_instances)*(1-pow((recount[3]/(recount[3]+drecount[3])),2)-pow((drecount[3]/(recount[3]+drecount[3])),2)))+
	(y_instances[2]/all_instances)*(1-pow((recount[4]/(recount[4]+drecount[4])),2)-pow((drecount[4]/(recount[4]+drecount[4])),2))+
	((n_instances[2]/all_instances)*(1-pow((recount[5]/(recount[5]+drecount[5])),2)-pow((drecount[5]/(recount[5]+drecount[5])),2)))+(y_instances[3]/all_instances)*
	(1-pow((recount[6]/(recount[6]+drecount[6])),2)-pow((drecount[6]/(recount[6]+drecount[6])),2)));
...
{
	int index=0;
	string alldata[30]={"Name","a","b","c","d","Recommend","1","0","1","1","Reservation","yes","no","yes","yes","Price Range","above 5k","below 5k"
	,"below 5k","below 5k","Cuisine","American","American","Desi","American","Timing","Lunch","Breakfast","Breakfast,Lunch","Breakfast,Lunch,Dinner"};
