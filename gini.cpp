#include<bits/stdc++.h>
using namespace std;
float find_gini(string alldata[],int arrlen){
	...
	for(int i=0;i<arrlen;i++){
		while(alldata[i]!='Recommend'){//loop to find where instance starts in a container of generic size
			instance_index++;
		}
		break;
	}
	for(int i=instance_index;i<arrlen;i++){
		if(alldata[i]=='1'){
			yes_instances++;
	...
	for(int i=0;i<arrlen;i++){
		while(alldata[i]!='Reservation'){//loop to find where instance starts in a container of generic size
			instance_index++;
		}
		break;
	}
	...
	for(int i=0;i<arrlen;i++){
		while(alldata[i]!='Price Range'){//loop to find where instance starts in a container of generic size
			instance_index++;
		}
		break;
	}
	...
	for(int i=0;i<arrlen;i++){
		while(alldata[i]!='Cuisine'){//loop to find where instance starts in a container of generic size
			instance_index++;
		}
		break;
	}
	...
	for(int i=0;i<arrlen;i++){
		while(alldata[i]!='Timing'){//loop to find where instance starts in a container of generic size
			instance_index++;
		}
		break;
	}
	...
	gini[4]=(1-(sqrt(yes_instances/all_instances))+(sqrt(no_instances/all_instances))+(sqrt(yes1_instances/all_instances))
	+(sqrt(no1_instances/all_instances))+(sqrt(yes2_instances/all_instances))+(sqrt(no2_instances/all_instances))+(sqrt(yes3_instances/all_instances)));
