...
int find_root_gini(string alldata[],int arrlen)
{
	...
	for(int i=indarr[4]+1;i<arrlen;i++){
	...
	+
	((n_instances[2]/all_instances)*(1-pow((recount[5]/(recount[5]+drecount[5])),2)-pow((drecount[5]/(recount[5]+drecount[5])),2)))
	+
	(y_instances[3]/all_instances)*(1-pow((recount[6]/(recount[6]+drecount[6])),2)-pow((drecount[6]/(recount[6]+drecount[6])),2))
	);
...
{
	int index=0;
	string alldata[30]={"Name","a","b","c","d","Recommend","1","0","1","1","Reservation","no","no","yes","yes","Price Range","above 5k","below 5k"
